...
import type { SpendingTotals } from "./spending-tables"
...
export function BudgetReport({ monthlyIncome, expenseTotals }: BudgetReportProps) {
  ...
  const halfYearDay = monthLengths.slice(0, 6).reduce((sum, length) => sum + length, 0) + 1 // primo giorno del mese 7
  ...
  return (
    <Card className="">
      <CardHeader>
        ...
        <CardDescription>Evoluzione del saldo nell'arco dei 12 mesi con eventi ricorrenti.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-md border p-4">
            ...
            <div className="text-2xl font-semibold">{formatCurrency(monthlyRemaining)}</div>
          </div>
          <div className="rounded-md border p-4">
            <div className="text-muted-foreground text-sm">Saldo annuale</div>
            <div className="text-2xl font-semibold">{formatCurrency(yearlyRemaining)}</div>
          </div>
        </div>
      </CardContent>
    </Card>
